...
class ProjectProgressBilling(models.Model):
    ...
    @api.depends('project_id', 'total_accomplishment', 'billing_accomplishment_ids', 'billing_history_ids')
    def _compute_billing_details(self):
        for i in self:
            if i.project_id:
                ...
                i.recoupment = i.project_id.downpayment_paid * (i.total_accomplishment / 100)
                total_billed = 0.0
                total_retention = 0.0
    ...
    partner_id = fields.Many2one('res.partner', string="Customer", compute='_compute_billing_details', store=True)
    ...
    recoupment_invoice_id = fields.Many2one('account.invoice', string="Invoice")
    invoiceable = fields.Monetary(string="Invoiceable", store=True, compute='_compute_billing_details')
    project_contract = fields.Monetary(string="Contract Value", store=True, compute='_compute_billing_details')
    previous_billed = fields.Monetary(string="Total Previously Billed", store=True, compute='_compute_billing_details')
    remaining_billable = fields.Monetary(string="Total Remaining Billable", store=True, compute='_compute_billing_details')
    ...
    invoice_state = fields.Selection([
        ...
        help=" * The 'Draft' status is used when a user is encoding a new and unconfirmed Invoice.\n"
             ...
             " * The 'Cancelled' status is used when user cancel invoice.", compute="_get_invoice_state")
    ...
    def compute_phase_accomplishment(self):
        for i in self:
            ...
            for phase in i.project_id.phase_ids:
                ...
                phase_accomplishment = (phase_accomplishment - sum([line.accomplishment for line in previous_accomplishement]))
                project_phase_accomplishment += (phase.phase_weight / total_phase_weight) * phase_phase_accomplishment
    ...
    def _prepare_invoice_line(self, type):
        ...
        res = {}
        if type == 'out_invoice':
            product = self.env.ref('construction_progress_billing.product_project_billing', raise_if_not_found=False)
            if not product:
                return False
            ...
            product = self.env.ref('construction_progress_billing.product_project_recoupment', raise_if_not_found=False)
            if not product:
                return False
    ...
    def create_billing_invoice(self):
        for i in self:
            ...
            if not i.recoupment_invoice_id or i.recoupment_invoice_id.state in ['cancel']:
                found = True
                i.write({
