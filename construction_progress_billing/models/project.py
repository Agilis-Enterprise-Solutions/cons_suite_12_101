...
class ProjectProjectionAccomplishment(models.Model):
    ...
    actual = fields.Float(string="Actual Percentage", store=True, compute="_get_actual_accomplishment")
...
class Project(models.Model):
    ...
    downpayment_paid = fields.Monetary(string="Downpayment Paid", readonly=True, states={'draft': [('readonly', False)]})
    billing_ids = fields.One2many('project.progress.billing', 'project_id', string="Progress Billing")
    downpayment_invoice_ids = fields.Many2many('account.invoice', 'project_downpayment_invoice_rel', 'project_id', 'invoice_id', string="Downpayment Invoice", readonly=True, states={'draft': [('readonly', False)]})
