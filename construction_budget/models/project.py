    ...
    def _get_document(self):
        for i in self:
            if i.task_id and i.phase_id:
                ...
                else: i.document = 'Project - Contract Supplement'
...
class Project(models.Model):
    ...
    @api.depends('project_budget_amount', 'phase_ids', 'phase_ids.phase_budget_amount', 'labor_budget_ids', 'overhead_budget_ids')
    def _get_budget_computation(self):
        for i in self:
            ...
            phase_budget = sum(line.phase_budget_amount for line in i.phase_ids)
            reserve = i.project_budget_amount - (labor_budget + phase_budget + overhead_budget)
            i.project_budget_allocated = (labor_budget + phase_budget + overhead_budget)
    ...
    @api.depends('task_ids', 'task_ids.material_budget', 'task_ids.service_budget', 'task_ids.overhead_budget', 'task_ids.equipment_budget', 'task_ids.labor_budget', 'labor_budget_ids', 'overhead_budget_ids')
    def _get_budget_summary(self):
        for i in self:
            material = service = equipment = 0.0
    ...
    project_contract_amount = fields.Monetary(String='Contract Amount', readonly=True, states={'draft': [('readonly', False)]})
    project_budget_amount = fields.Monetary(string="Budget Amount", readonly=True, states={'draft': [('readonly', False)]})
    ...
    material_budget = fields.Monetary(string="Material Budget", store=True, compute="_get_budget_summary")
    service_budget= fields.Monetary(string="Service Budget", store=True, compute="_get_budget_summary")
    labor_budget = fields.Monetary(string="Labor Budget", store=True, compute="_get_budget_summary")
    equipment_budget = fields.Monetary(string="Equipment Budget", store=True, compute="_get_budget_summary")
    overhead_budget = fields.Monetary(string="Overhead Budget", store=True, compute="_get_budget_summary")
    total_budget = fields.Monetary(string="Total Budget", store=True, compute="_get_budget_summary")
